    ...
    class Solver : Validator
    {
        ...
        public List<int[]> IsCorrect(CellsGrid inputGrid)
        {
            ...
            for (int x = 0; x < 9; x++) for (int y = 0; y < 9; y++)
                {
                    if (inputGrid[x, y] != 0 && SolvedGrid[x, y] != inputGrid[x, y]) output.Add(new int[] { x, y });
                }
            return output;
        }
        ...
        private bool RemoveHidden(int x, int y, int type = 2)
        {
            ...
            foreach (List<int> combination in numbers)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    if (GetPossibleRow(x)[i].Any(d => combination.Contains(d))) row.Add(i);
                    if (GetPossibleColumn(y)[i].Any(d => combination.Contains(d))) column.Add(i);
                    if (GetPossibleSubgrid(x, y)[i].Any(d => combination.Contains(d))) subgrid++;
                }
                ...
                if (row.Count == type && subgrid <= type)
                {
                    foreach (int item in row)
                    {
                        possible[x][item] = combination;
                        if (debug) Debug.Print("Hidden row " + x + " " + item + " num " + combination[0] + ", " + combination[1]);
                    }
                    return true;
                }
                if (column.Count == type && subgrid <= type)
                {
                    foreach (int item in column)
                    {
                        possible[item][y] = combination;
                        if (debug) Debug.Print("Hidden col " + item + " " + y + " num " + combination[0] + ", " + combination[1]);
                    }
                    return true;
                }/* Not 100% OK
        ...
        /// <param name="type">How many possible numbers and cells</param>
        /// <returns>True if anything was removed.</returns>
        private bool RemoveSame(int x, int y, int type = 2)
        {
            List<List<int>> numbers = new List<List<int>>();
            int n = possible[x][y].Count;
            if (type >= n) return false;
            ...
            foreach (List<int> combination in numbers)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    ...
                    if (GetPossibleSubgrid(x, y)[i].All(d => combination.Contains(d))) subgrid++;
                }
